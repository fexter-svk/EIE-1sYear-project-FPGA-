// ----------------------------------------------------------------------
//  HLS HDL:        Verilog Netlister
//  HLS Version:    2011a.126 Production Release
//  HLS Date:       Wed Aug  8 00:52:07 PDT 2012
// 
//  Generated by:   mf2915@EEWS305-026
//  Generated date: Thu May 05 11:57:52 2016
// ----------------------------------------------------------------------

// 
// ------------------------------------------------------------------
//  Design Unit:    markers_core
// ------------------------------------------------------------------


module markers_core (
  vin, vga_xy, clk, en, arst_n, vout_rsc_mgc_out_stdreg_d
);
  input [89:0] vin;
  input [19:0] vga_xy;
  input clk;
  input en;
  input arst_n;
  output [29:0] vout_rsc_mgc_out_stdreg_d;
  reg [29:0] vout_rsc_mgc_out_stdreg_d;



  // Interconnect Declarations for Component Instantiations 
  always @(*)
  begin : core
    // Interconnect Declarations
    reg [3:0] acc_6_0_sva;
    reg [3:0] acc_6_1_sva;
    reg [9:0] red_xy_0_sva;
    reg [9:0] red_xy_1_sva;
    reg [9:0] blue_xy_0_sva;
    reg [9:0] blue_xy_1_sva;
    reg [9:0] red_xy_previous_0_sva;
    reg [9:0] red_xy_previous_1_sva;
    reg [9:0] blue_xy_previous_0_sva;
    reg [9:0] blue_xy_previous_1_sva;
    reg [5:0] acc_imod_sva;
    reg [3:0] if_acc_1_psp_sva;
    reg [3:0] if_acc_svs;
    reg unequal_tmp;
    reg land_sva_1;
    reg land_lpi_1_dfm;
    reg slc_svs;
    reg land_3_sva_1;
    reg land_3_lpi_1_dfm;
    reg [3:0] acc_6_0_sva_2;
    reg else_3_aif_slc_svs;
    reg else_3_land_1_sva_1;
    reg else_3_land_1_lpi_1_dfm;
    reg else_3_land_sva_1;
    reg else_3_land_lpi_1_dfm;
    reg [3:0] acc_6_1_sva_2;
    reg [3:0] acc_6_0_sva_dfm_1;
    reg [3:0] acc_6_1_sva_dfm_2;
    reg if_5_land_lpi_1_dfm;
    reg [9:0] red_xy_0_sva_2;
    reg [9:0] red_xy_1_sva_2;
    reg [9:0] red_xy_0_sva_dfm_2;
    reg [9:0] red_xy_1_sva_dfm_2;
    reg if_7_land_lpi_1_dfm;
    reg [9:0] blue_xy_0_sva_2;
    reg [9:0] blue_xy_1_sva_2;
    reg [9:0] blue_xy_0_sva_dfm_2;
    reg [9:0] blue_xy_1_sva_dfm_2;
    reg land_5_lpi_1_dfm;
    reg [10:0] deltax_red_acc_psp_sva;
    reg slc_3_svs;
    reg land_6_sva_1;
    reg [9:0] red_xy_previous_0_sva_dfm_1;
    reg [9:0] red_xy_previous_1_sva_dfm_1;
    reg [10:0] deltax_square_red_acc_psp_sva;
    reg land_7_lpi_1_dfm;
    reg [10:0] deltax_blue_acc_psp_sva;
    reg slc_4_svs;
    reg land_8_sva_1;
    reg [9:0] blue_xy_previous_0_sva_dfm_1;
    reg [9:0] blue_xy_previous_1_sva_dfm_1;
    reg land_10_sva_1;
    reg land_10_lpi_1_dfm;
    reg [10:0] deltay_square_red_acc_psp_sva;
    reg aif_37_land_sva_1;
    reg land_9_lpi_1_dfm;
    reg [10:0] deltax_square_blue_acc_psp_sva;
    reg else_13_land_1_sva_1;
    reg else_13_land_1_lpi_1_dfm;
    reg [10:0] deltay_square_blue_acc_psp_sva;
    reg else_13_aif_1_land_sva_1;
    reg [3:0] acc_6_0_sva_dfm_2;
    reg [3:0] acc_6_1_sva_dfm_3;
    reg [9:0] red_xy_0_sva_dfm_3;
    reg [9:0] red_xy_1_sva_dfm_3;
    reg [9:0] blue_xy_0_sva_dfm_3;
    reg [9:0] blue_xy_1_sva_dfm_3;
    reg land_3_lpi_1_dfm_1;
    reg else_3_land_1_lpi_1_dfm_2;
    reg [9:0] red_xy_0_sva_dfm_4;
    reg [9:0] red_xy_1_sva_dfm_4;
    reg [9:0] blue_xy_0_sva_dfm_4;
    reg [9:0] blue_xy_1_sva_dfm_4;
    reg land_5_lpi_1_dfm_1;
    reg [10:0] deltax_red_acc_psp_sva_1;
    reg slc_3_svs_1;
    reg [9:0] red_xy_previous_1_sva_dfm_2;
    reg land_7_lpi_1_dfm_1;
    reg [10:0] deltax_blue_acc_psp_sva_1;
    reg [9:0] blue_xy_previous_0_sva_dfm_2;
    reg [9:0] blue_xy_previous_1_sva_dfm_2;
    reg [9:0] blue_xy_previous_1_sva_dfm_3;
    reg land_10_lpi_1_dfm_1;
    reg land_9_lpi_1_dfm_1;
    reg else_3_land_1_lpi_1_dfm_st;
    reg slc_3_svs_st;
    reg land_10_lpi_1_dfm_st;
    reg land_9_lpi_1_dfm_st;
    reg else_13_slc_1_itm;
    reg [9:0] deltax_square_blue_slc_deltax_square_blue_acc_psp_itm;
    reg [9:0] deltax_square_blue_slc_deltax_square_blue_acc_psp_itm_1;
    reg else_13_slc_itm;
    reg else_13_slc_itm_1;
    reg land_3_lpi_1_dfm_st_1;
    reg else_3_land_1_lpi_1_dfm_st_1;
    reg slc_3_svs_st_1;
    reg land_10_lpi_1_dfm_st_1;
    reg land_9_lpi_1_dfm_st_1;
    reg else_13_slc_1_itm_1;
    reg main_stage_0_2;
    reg main_stage_0_3;
    reg main_stage_0_4;
    reg main_stage_0_5;
    reg or_7_cse;
    reg or_9_cse;
    reg nand_cse;
    reg nand_4_cse;

    reg[9:0] mux_12_nl;
    reg[9:0] mux_16_nl;
    reg[9:0] select_0;
    begin : core_rlpExit
      forever begin : core_rlp
        // C-Step 0 of Loop 'core_rlp'
        acc_6_0_sva = 4'b0;
        acc_6_1_sva = 4'b0;
        red_xy_0_sva = 10'b0;
        red_xy_1_sva = 10'b0;
        blue_xy_0_sva = 10'b0;
        blue_xy_1_sva = 10'b0;
        red_xy_previous_0_sva = 10'b0;
        red_xy_previous_1_sva = 10'b0;
        blue_xy_previous_0_sva = 10'b0;
        blue_xy_previous_1_sva = 10'b0;
        main_stage_0_2 = 1'b0;
        main_stage_0_3 = 1'b0;
        main_stage_0_4 = 1'b0;
        main_stage_0_5 = 1'b0;
        begin : mainExit
          forever begin : main
            // C-Step 0 of Loop 'main'
            begin : waitLoop0Exit
              forever begin : waitLoop0
                @(posedge clk or negedge ( arst_n ));
                if ( ~ arst_n )
                  disable core_rlpExit;
                if ( en )
                  disable waitLoop0Exit;
              end
            end
            // C-Step 1 of Loop 'main'
            if ( main_stage_0_5 ) begin
              else_13_aif_1_land_sva_1 = 1'b0;
              else_13_land_1_sva_1 = 1'b0;
              if ( land_9_lpi_1_dfm_st_1 ) begin
              end
              else begin
                if ( else_13_slc_1_itm_1 ) begin
                end
                else begin
                  else_13_land_1_sva_1 = ~ (readslicef_11_1_10((({1'b1 , (~ deltax_square_blue_slc_deltax_square_blue_acc_psp_itm_1)})
                      + 11'b101001)));
                end
                else_13_land_1_lpi_1_dfm = else_13_land_1_sva_1 & (~ else_13_slc_itm_1);
                if ( else_13_land_1_lpi_1_dfm ) begin
                  deltay_square_blue_acc_psp_sva = readslicef_12_11_1((({1'b1 , (vga_xy[19:10])
                      , 1'b1}) + conv_u2s_11_12({(~ blue_xy_previous_1_sva_dfm_3)
                      , 1'b1})));
                  if ( deltay_square_blue_acc_psp_sva[10] ) begin
                  end
                  else begin
                    else_13_aif_1_land_sva_1 = ~ (readslicef_11_1_10((({1'b1 , (~
                        (deltay_square_blue_acc_psp_sva[9:0]))}) + 11'b101001)));
                  end
                end
              end
              vout_rsc_mgc_out_stdreg_d <= {({{9{land_9_lpi_1_dfm_1}}, land_9_lpi_1_dfm_1})
                  , 10'b0 , (signext_10_1(else_13_aif_1_land_sva_1 & (~ (deltay_square_blue_acc_psp_sva[10]))
                  & else_13_land_1_lpi_1_dfm & (~ land_9_lpi_1_dfm_1)))};
            end
            if ( main_stage_0_4 ) begin
              aif_37_land_sva_1 = 1'b0;
              if ( land_10_lpi_1_dfm_st_1 ) begin
                deltay_square_red_acc_psp_sva = readslicef_12_11_1((({1'b1 , (vga_xy[19:10])
                    , 1'b1}) + conv_u2s_11_12({(~ red_xy_previous_1_sva_dfm_2) ,
                    1'b1})));
                if ( deltay_square_red_acc_psp_sva[10] ) begin
                end
                else begin
                  aif_37_land_sva_1 = ~ (readslicef_11_1_10((({1'b1 , (~ (deltay_square_red_acc_psp_sva[9:0]))})
                      + 11'b101001)));
                end
              end
              land_9_lpi_1_dfm = aif_37_land_sva_1 & (~ (deltay_square_red_acc_psp_sva[10]))
                  & land_10_lpi_1_dfm_1;
              land_9_lpi_1_dfm_st = land_9_lpi_1_dfm;
              if ( land_9_lpi_1_dfm ) begin
              end
              else begin
                deltax_square_blue_acc_psp_sva = readslicef_12_11_1((({1'b1 , (vga_xy[9:0])
                    , 1'b1}) + conv_u2s_11_12({(~ blue_xy_previous_0_sva_dfm_2) ,
                    1'b1})));
                else_13_slc_1_itm = deltax_square_blue_acc_psp_sva[10];
                if ( deltax_square_blue_acc_psp_sva[10] ) begin
                end
                else begin
                  deltax_square_blue_slc_deltax_square_blue_acc_psp_itm = deltax_square_blue_acc_psp_sva[9:0];
                end
                else_13_slc_itm = deltax_square_blue_acc_psp_sva[10];
              end
              blue_xy_previous_1_sva = blue_xy_previous_1_sva_dfm_2;
            end
            if ( main_stage_0_3 ) begin
              land_10_sva_1 = 1'b0;
              land_8_sva_1 = 1'b0;
              land_6_sva_1 = 1'b0;
              if ( slc_3_svs_st_1 ) begin
                land_6_sva_1 = readslicef_12_1_11((conv_s2s_11_12(~ deltax_red_acc_psp_sva_1)
                    + 12'b111111101101));
              end
              or_7_cse = land_5_lpi_1_dfm_1 | (land_6_sva_1 & slc_3_svs_1);
              red_xy_previous_0_sva_dfm_1 = MUX_v_10_2_2({red_xy_previous_0_sva ,
                  red_xy_0_sva_dfm_4}, or_7_cse);
              red_xy_previous_1_sva_dfm_1 = MUX_v_10_2_2({red_xy_previous_1_sva ,
                  red_xy_1_sva_dfm_4}, or_7_cse);
              deltax_square_red_acc_psp_sva = readslicef_12_11_1((({1'b1 , (vga_xy[9:0])
                  , 1'b1}) + conv_u2s_11_12({(~ red_xy_previous_0_sva_dfm_1) , 1'b1})));
              slc_4_svs = readslicef_10_1_9((conv_s2u_9_10(deltax_blue_acc_psp_sva_1[10:2])
                  + 10'b1111111011));
              if ( slc_4_svs ) begin
                land_8_sva_1 = readslicef_12_1_11((conv_s2s_11_12(~ deltax_blue_acc_psp_sva_1)
                    + 12'b111111101101));
              end
              or_9_cse = land_7_lpi_1_dfm_1 | (land_8_sva_1 & slc_4_svs);
              blue_xy_previous_0_sva_dfm_1 = MUX_v_10_2_2({blue_xy_previous_0_sva
                  , blue_xy_0_sva_dfm_4}, or_9_cse);
              blue_xy_previous_1_sva_dfm_1 = MUX_v_10_2_2({blue_xy_previous_1_sva
                  , blue_xy_1_sva_dfm_4}, or_9_cse);
              if ( deltax_square_red_acc_psp_sva[10] ) begin
              end
              else begin
                land_10_sva_1 = ~ (readslicef_11_1_10((({1'b1 , (~ (deltax_square_red_acc_psp_sva[9:0]))})
                    + 11'b101001)));
              end
              land_10_lpi_1_dfm = land_10_sva_1 & (~ (deltax_square_red_acc_psp_sva[10]));
              land_10_lpi_1_dfm_st = land_10_lpi_1_dfm;
              red_xy_previous_0_sva = red_xy_previous_0_sva_dfm_1;
              red_xy_previous_1_sva = red_xy_previous_1_sva_dfm_1;
              blue_xy_previous_0_sva = blue_xy_previous_0_sva_dfm_1;
            end
            if ( main_stage_0_2 ) begin
              blue_xy_1_sva_2 = 10'b0;
              blue_xy_0_sva_2 = 10'b0;
              red_xy_1_sva_2 = 10'b0;
              red_xy_0_sva_2 = 10'b0;
              acc_6_1_sva_2 = 4'b0;
              else_3_land_sva_1 = 1'b0;
              acc_6_0_sva_2 = 4'b0;
              if ( land_3_lpi_1_dfm_st_1 ) begin
                acc_6_0_sva_2 = acc_6_0_sva_dfm_2 + 4'b1;
              end
              else begin
                if ( else_3_land_1_lpi_1_dfm_st_1 ) begin
                  else_3_land_sva_1 = ~ (readslicef_8_1_7((conv_u2u_7_8(vin[9:3])
                      + 8'b10100001)));
                end
                else_3_land_lpi_1_dfm = else_3_land_sva_1 & else_3_land_1_lpi_1_dfm_2;
                if ( else_3_land_lpi_1_dfm ) begin
                  acc_6_1_sva_2 = acc_6_1_sva_dfm_3 + 4'b1;
                end
              end
              acc_6_0_sva_dfm_1 = MUX_v_4_2_2({acc_6_0_sva_dfm_2 , acc_6_0_sva_2},
                  land_3_lpi_1_dfm_1);
              acc_6_1_sva_dfm_2 = MUX_v_4_2_2({acc_6_1_sva_dfm_3 , acc_6_1_sva_2},
                  else_3_land_lpi_1_dfm & (~ land_3_lpi_1_dfm_1));
              if ( acc_6_0_sva_dfm_1[3] ) begin
                if_5_land_lpi_1_dfm = ~((red_xy_1_sva_dfm_3[9]) | (red_xy_1_sva_dfm_3[8])
                    | (red_xy_1_sva_dfm_3[7]) | (red_xy_1_sva_dfm_3[6]) | (red_xy_1_sva_dfm_3[5])
                    | (red_xy_1_sva_dfm_3[4]) | (red_xy_1_sva_dfm_3[3]) | (red_xy_1_sva_dfm_3[2])
                    | (red_xy_1_sva_dfm_3[1]) | (red_xy_1_sva_dfm_3[0]) | (red_xy_0_sva_dfm_3[9])
                    | (red_xy_0_sva_dfm_3[8]) | (red_xy_0_sva_dfm_3[7]) | (red_xy_0_sva_dfm_3[6])
                    | (red_xy_0_sva_dfm_3[5]) | (red_xy_0_sva_dfm_3[4]) | (red_xy_0_sva_dfm_3[3])
                    | (red_xy_0_sva_dfm_3[2]) | (red_xy_0_sva_dfm_3[1]) | (red_xy_0_sva_dfm_3[0]));
                if ( if_5_land_lpi_1_dfm ) begin
                  red_xy_0_sva_2 = vga_xy[9:0];
                  red_xy_1_sva_2 = vga_xy[19:10];
                end
              end
              nand_cse = ~((~((~ if_5_land_lpi_1_dfm) & (acc_6_0_sva_dfm_1[3])))
                  & (acc_6_0_sva_dfm_1[3]));
              red_xy_0_sva_dfm_2 = MUX_v_10_2_2({red_xy_0_sva_2 , red_xy_0_sva_dfm_3},
                  nand_cse);
              red_xy_1_sva_dfm_2 = MUX_v_10_2_2({red_xy_1_sva_2 , red_xy_1_sva_dfm_3},
                  nand_cse);
              if ( acc_6_1_sva_dfm_2[3] ) begin
                if_7_land_lpi_1_dfm = ~((blue_xy_1_sva_dfm_3[9]) | (blue_xy_1_sva_dfm_3[8])
                    | (blue_xy_1_sva_dfm_3[7]) | (blue_xy_1_sva_dfm_3[6]) | (blue_xy_1_sva_dfm_3[5])
                    | (blue_xy_1_sva_dfm_3[4]) | (blue_xy_1_sva_dfm_3[3]) | (blue_xy_1_sva_dfm_3[2])
                    | (blue_xy_1_sva_dfm_3[1]) | (blue_xy_1_sva_dfm_3[0]) | (blue_xy_0_sva_dfm_3[9])
                    | (blue_xy_0_sva_dfm_3[8]) | (blue_xy_0_sva_dfm_3[7]) | (blue_xy_0_sva_dfm_3[6])
                    | (blue_xy_0_sva_dfm_3[5]) | (blue_xy_0_sva_dfm_3[4]) | (blue_xy_0_sva_dfm_3[3])
                    | (blue_xy_0_sva_dfm_3[2]) | (blue_xy_0_sva_dfm_3[1]) | (blue_xy_0_sva_dfm_3[0]));
                if ( if_7_land_lpi_1_dfm ) begin
                  blue_xy_0_sva_2 = vga_xy[9:0];
                  blue_xy_1_sva_2 = vga_xy[19:10];
                end
              end
              nand_4_cse = ~((~((~ if_7_land_lpi_1_dfm) & (acc_6_1_sva_dfm_2[3])))
                  & (acc_6_1_sva_dfm_2[3]));
              blue_xy_0_sva_dfm_2 = MUX_v_10_2_2({blue_xy_0_sva_2 , blue_xy_0_sva_dfm_3},
                  nand_4_cse);
              blue_xy_1_sva_dfm_2 = MUX_v_10_2_2({blue_xy_1_sva_2 , blue_xy_1_sva_dfm_3},
                  nand_4_cse);
              land_5_lpi_1_dfm = ~((~((red_xy_0_sva_dfm_2[9]) | (red_xy_0_sva_dfm_2[8])
                  | (red_xy_0_sva_dfm_2[7]) | (red_xy_0_sva_dfm_2[6]) | (red_xy_0_sva_dfm_2[5])
                  | (red_xy_0_sva_dfm_2[4]) | (red_xy_0_sva_dfm_2[3]) | (red_xy_0_sva_dfm_2[2])
                  | (red_xy_0_sva_dfm_2[1]) | (red_xy_0_sva_dfm_2[0]))) | (red_xy_previous_0_sva[9])
                  | (red_xy_previous_0_sva[8]) | (red_xy_previous_0_sva[7]) | (red_xy_previous_0_sva[6])
                  | (red_xy_previous_0_sva[5]) | (red_xy_previous_0_sva[4]) | (red_xy_previous_0_sva[3])
                  | (red_xy_previous_0_sva[2]) | (red_xy_previous_0_sva[1]) | (red_xy_previous_0_sva[0]));
              mux_12_nl = MUX_v_10_2_2({red_xy_previous_0_sva , red_xy_0_sva_dfm_2},
                  land_5_lpi_1_dfm);
              deltax_red_acc_psp_sva = readslicef_12_11_1((({1'b1 , red_xy_0_sva_dfm_2
                  , 1'b1}) + conv_u2s_11_12({(~ (mux_12_nl)) , 1'b1})));
              slc_3_svs = readslicef_10_1_9((conv_s2u_9_10(deltax_red_acc_psp_sva[10:2])
                  + 10'b1111111011));
              slc_3_svs_st = slc_3_svs;
              land_7_lpi_1_dfm = ~((~((blue_xy_0_sva_dfm_2[9]) | (blue_xy_0_sva_dfm_2[8])
                  | (blue_xy_0_sva_dfm_2[7]) | (blue_xy_0_sva_dfm_2[6]) | (blue_xy_0_sva_dfm_2[5])
                  | (blue_xy_0_sva_dfm_2[4]) | (blue_xy_0_sva_dfm_2[3]) | (blue_xy_0_sva_dfm_2[2])
                  | (blue_xy_0_sva_dfm_2[1]) | (blue_xy_0_sva_dfm_2[0]))) | (blue_xy_previous_0_sva[9])
                  | (blue_xy_previous_0_sva[8]) | (blue_xy_previous_0_sva[7]) | (blue_xy_previous_0_sva[6])
                  | (blue_xy_previous_0_sva[5]) | (blue_xy_previous_0_sva[4]) | (blue_xy_previous_0_sva[3])
                  | (blue_xy_previous_0_sva[2]) | (blue_xy_previous_0_sva[1]) | (blue_xy_previous_0_sva[0]));
              mux_16_nl = MUX_v_10_2_2({blue_xy_previous_0_sva , blue_xy_0_sva_dfm_2},
                  land_7_lpi_1_dfm);
              deltax_blue_acc_psp_sva = readslicef_12_11_1((({1'b1 , blue_xy_0_sva_dfm_2
                  , 1'b1}) + conv_u2s_11_12({(~ (mux_16_nl)) , 1'b1})));
              acc_6_0_sva = acc_6_0_sva_dfm_1;
              acc_6_1_sva = acc_6_1_sva_dfm_2;
              red_xy_0_sva = red_xy_0_sva_dfm_2;
              red_xy_1_sva = red_xy_1_sva_dfm_2;
              blue_xy_0_sva = blue_xy_0_sva_dfm_2;
              blue_xy_1_sva = blue_xy_1_sva_dfm_2;
            end
            else_3_land_1_sva_1 = 1'b0;
            land_3_sva_1 = 1'b0;
            land_sva_1 = 1'b0;
            acc_imod_sva = conv_s2s_5_6(({3'b100 , (vga_xy[9:8])}) + conv_u2s_4_5(readslicef_5_4_1((conv_u2u_4_5({(vga_xy[2:0])
                , 1'b1}) + conv_u2u_4_5(vga_xy[6:3]))))) + conv_u2s_5_6({(conv_u2u_1_2(~
                (vga_xy[3])) + conv_u2u_1_2(~ (vga_xy[7]))) , 2'b0 , (readslicef_2_1_1((conv_u2u_1_2(vga_xy[7])
                + 2'b1)))});
            if_acc_1_psp_sva = (readslicef_5_4_1((({(~ (acc_imod_sva[3])) , 4'b1})
                + conv_s2u_3_5(acc_imod_sva[5:3])))) + ({1'b1 , (acc_imod_sva[2:0])});
            if_acc_svs = conv_s2u_1_4(if_acc_1_psp_sva[3]) + if_acc_1_psp_sva;
            unequal_tmp = (if_acc_svs[3]) | (if_acc_svs[2]) | (if_acc_svs[1]) | (if_acc_svs[0]);
            select_0 = vga_xy[19:10];
            case (select_0)
              10'b0 : begin
                land_sva_1 = ~((vga_xy[9]) | (vga_xy[8]) | (vga_xy[7]) | (vga_xy[6])
                    | (vga_xy[5]) | (vga_xy[4]) | (vga_xy[3]) | (vga_xy[2]) | (vga_xy[1])
                    | (vga_xy[0]));
              end
              default : begin
                // NOP
              end
            endcase
            land_lpi_1_dfm = land_sva_1 & (~((vga_xy[19]) | (vga_xy[18]) | (vga_xy[17])
                | (vga_xy[16]) | (vga_xy[15]) | (vga_xy[14]) | (vga_xy[13]) | (vga_xy[12])
                | (vga_xy[11]) | (vga_xy[10])));
            slc_svs = readslicef_8_1_7((conv_u2u_7_8(vin[29:23]) + 8'b10100001));
            if ( slc_svs ) begin
            end
            else begin
              land_3_sva_1 = ~ (readslicef_9_1_8((({1'b1 , (~ (vin[19:12]))}) + 9'b1011011)));
            end
            land_3_lpi_1_dfm = land_3_sva_1 & (~ slc_svs);
            if ( land_3_lpi_1_dfm ) begin
            end
            else begin
              else_3_aif_slc_svs = readslicef_9_1_8((({1'b1 , (~ (vin[29:22]))})
                  + 9'b1011011));
              if ( else_3_aif_slc_svs ) begin
              end
              else begin
                else_3_land_1_sva_1 = ~ (readslicef_9_1_8((({1'b1 , (~ (vin[19:12]))})
                    + 9'b1011011)));
              end
              else_3_land_1_lpi_1_dfm = else_3_land_1_sva_1 & (~ else_3_aif_slc_svs);
              else_3_land_1_lpi_1_dfm_st = else_3_land_1_lpi_1_dfm;
            end
            acc_6_0_sva_dfm_2 = acc_6_0_sva & ({{3{unequal_tmp}}, unequal_tmp});
            acc_6_1_sva_dfm_3 = acc_6_1_sva & ({{3{unequal_tmp}}, unequal_tmp});
            red_xy_0_sva_dfm_3 = red_xy_0_sva & (signext_10_1(~ land_lpi_1_dfm));
            red_xy_1_sva_dfm_3 = red_xy_1_sva & (signext_10_1(~ land_lpi_1_dfm));
            blue_xy_0_sva_dfm_3 = blue_xy_0_sva & (signext_10_1(~ land_lpi_1_dfm));
            blue_xy_1_sva_dfm_3 = blue_xy_1_sva & (signext_10_1(~ land_lpi_1_dfm));
            land_3_lpi_1_dfm_1 = land_3_lpi_1_dfm;
            else_3_land_1_lpi_1_dfm_2 = else_3_land_1_lpi_1_dfm;
            red_xy_0_sva_dfm_4 = red_xy_0_sva_dfm_2;
            red_xy_1_sva_dfm_4 = red_xy_1_sva_dfm_2;
            blue_xy_0_sva_dfm_4 = blue_xy_0_sva_dfm_2;
            blue_xy_1_sva_dfm_4 = blue_xy_1_sva_dfm_2;
            land_5_lpi_1_dfm_1 = land_5_lpi_1_dfm;
            deltax_red_acc_psp_sva_1 = deltax_red_acc_psp_sva;
            slc_3_svs_1 = slc_3_svs;
            red_xy_previous_1_sva_dfm_2 = red_xy_previous_1_sva_dfm_1;
            land_7_lpi_1_dfm_1 = land_7_lpi_1_dfm;
            deltax_blue_acc_psp_sva_1 = deltax_blue_acc_psp_sva;
            blue_xy_previous_0_sva_dfm_2 = blue_xy_previous_0_sva_dfm_1;
            blue_xy_previous_1_sva_dfm_3 = blue_xy_previous_1_sva_dfm_2;
            blue_xy_previous_1_sva_dfm_2 = blue_xy_previous_1_sva_dfm_1;
            land_10_lpi_1_dfm_1 = land_10_lpi_1_dfm;
            land_9_lpi_1_dfm_1 = land_9_lpi_1_dfm;
            deltax_square_blue_slc_deltax_square_blue_acc_psp_itm_1 = deltax_square_blue_slc_deltax_square_blue_acc_psp_itm;
            else_13_slc_itm_1 = else_13_slc_itm;
            land_3_lpi_1_dfm_st_1 = land_3_lpi_1_dfm;
            else_3_land_1_lpi_1_dfm_st_1 = else_3_land_1_lpi_1_dfm_st;
            slc_3_svs_st_1 = slc_3_svs_st;
            land_10_lpi_1_dfm_st_1 = land_10_lpi_1_dfm_st;
            land_9_lpi_1_dfm_st_1 = land_9_lpi_1_dfm_st;
            else_13_slc_1_itm_1 = else_13_slc_1_itm;
            main_stage_0_5 = main_stage_0_4;
            main_stage_0_4 = main_stage_0_3;
            main_stage_0_3 = main_stage_0_2;
            main_stage_0_2 = 1'b1;
          end
        end
      end
    end
    nand_4_cse = 1'b0;
    nand_cse = 1'b0;
    or_9_cse = 1'b0;
    or_7_cse = 1'b0;
    main_stage_0_5 = 1'b0;
    main_stage_0_4 = 1'b0;
    main_stage_0_3 = 1'b0;
    main_stage_0_2 = 1'b0;
    else_13_slc_1_itm_1 = 1'b0;
    land_9_lpi_1_dfm_st_1 = 1'b0;
    land_10_lpi_1_dfm_st_1 = 1'b0;
    slc_3_svs_st_1 = 1'b0;
    else_3_land_1_lpi_1_dfm_st_1 = 1'b0;
    land_3_lpi_1_dfm_st_1 = 1'b0;
    else_13_slc_itm_1 = 1'b0;
    else_13_slc_itm = 1'b0;
    deltax_square_blue_slc_deltax_square_blue_acc_psp_itm_1 = 10'b0;
    deltax_square_blue_slc_deltax_square_blue_acc_psp_itm = 10'b0;
    else_13_slc_1_itm = 1'b0;
    land_9_lpi_1_dfm_st = 1'b0;
    land_10_lpi_1_dfm_st = 1'b0;
    slc_3_svs_st = 1'b0;
    else_3_land_1_lpi_1_dfm_st = 1'b0;
    land_9_lpi_1_dfm_1 = 1'b0;
    land_10_lpi_1_dfm_1 = 1'b0;
    blue_xy_previous_1_sva_dfm_3 = 10'b0;
    blue_xy_previous_1_sva_dfm_2 = 10'b0;
    blue_xy_previous_0_sva_dfm_2 = 10'b0;
    deltax_blue_acc_psp_sva_1 = 11'b0;
    land_7_lpi_1_dfm_1 = 1'b0;
    red_xy_previous_1_sva_dfm_2 = 10'b0;
    slc_3_svs_1 = 1'b0;
    deltax_red_acc_psp_sva_1 = 11'b0;
    land_5_lpi_1_dfm_1 = 1'b0;
    blue_xy_1_sva_dfm_4 = 10'b0;
    blue_xy_0_sva_dfm_4 = 10'b0;
    red_xy_1_sva_dfm_4 = 10'b0;
    red_xy_0_sva_dfm_4 = 10'b0;
    else_3_land_1_lpi_1_dfm_2 = 1'b0;
    land_3_lpi_1_dfm_1 = 1'b0;
    blue_xy_1_sva_dfm_3 = 10'b0;
    blue_xy_0_sva_dfm_3 = 10'b0;
    red_xy_1_sva_dfm_3 = 10'b0;
    red_xy_0_sva_dfm_3 = 10'b0;
    acc_6_1_sva_dfm_3 = 4'b0;
    acc_6_0_sva_dfm_2 = 4'b0;
    else_13_aif_1_land_sva_1 = 1'b0;
    deltay_square_blue_acc_psp_sva = 11'b0;
    else_13_land_1_lpi_1_dfm = 1'b0;
    else_13_land_1_sva_1 = 1'b0;
    deltax_square_blue_acc_psp_sva = 11'b0;
    land_9_lpi_1_dfm = 1'b0;
    aif_37_land_sva_1 = 1'b0;
    deltay_square_red_acc_psp_sva = 11'b0;
    land_10_lpi_1_dfm = 1'b0;
    land_10_sva_1 = 1'b0;
    blue_xy_previous_1_sva_dfm_1 = 10'b0;
    blue_xy_previous_0_sva_dfm_1 = 10'b0;
    land_8_sva_1 = 1'b0;
    slc_4_svs = 1'b0;
    deltax_blue_acc_psp_sva = 11'b0;
    land_7_lpi_1_dfm = 1'b0;
    deltax_square_red_acc_psp_sva = 11'b0;
    red_xy_previous_1_sva_dfm_1 = 10'b0;
    red_xy_previous_0_sva_dfm_1 = 10'b0;
    land_6_sva_1 = 1'b0;
    slc_3_svs = 1'b0;
    deltax_red_acc_psp_sva = 11'b0;
    land_5_lpi_1_dfm = 1'b0;
    blue_xy_1_sva_dfm_2 = 10'b0;
    blue_xy_0_sva_dfm_2 = 10'b0;
    blue_xy_1_sva_2 = 10'b0;
    blue_xy_0_sva_2 = 10'b0;
    if_7_land_lpi_1_dfm = 1'b0;
    red_xy_1_sva_dfm_2 = 10'b0;
    red_xy_0_sva_dfm_2 = 10'b0;
    red_xy_1_sva_2 = 10'b0;
    red_xy_0_sva_2 = 10'b0;
    if_5_land_lpi_1_dfm = 1'b0;
    acc_6_1_sva_dfm_2 = 4'b0;
    acc_6_0_sva_dfm_1 = 4'b0;
    acc_6_1_sva_2 = 4'b0;
    else_3_land_lpi_1_dfm = 1'b0;
    else_3_land_sva_1 = 1'b0;
    else_3_land_1_lpi_1_dfm = 1'b0;
    else_3_land_1_sva_1 = 1'b0;
    else_3_aif_slc_svs = 1'b0;
    acc_6_0_sva_2 = 4'b0;
    land_3_lpi_1_dfm = 1'b0;
    land_3_sva_1 = 1'b0;
    slc_svs = 1'b0;
    land_lpi_1_dfm = 1'b0;
    land_sva_1 = 1'b0;
    unequal_tmp = 1'b0;
    if_acc_svs = 4'b0;
    if_acc_1_psp_sva = 4'b0;
    acc_imod_sva = 6'b0;
    blue_xy_previous_1_sva = 10'b0;
    blue_xy_previous_0_sva = 10'b0;
    red_xy_previous_1_sva = 10'b0;
    red_xy_previous_0_sva = 10'b0;
    blue_xy_1_sva = 10'b0;
    blue_xy_0_sva = 10'b0;
    red_xy_1_sva = 10'b0;
    red_xy_0_sva = 10'b0;
    acc_6_1_sva = 4'b0;
    acc_6_0_sva = 4'b0;
    vout_rsc_mgc_out_stdreg_d <= 30'b0;
  end


  function [0:0] readslicef_11_1_10;
    input [10:0] vector;
    reg [10:0] tmp;
  begin
    tmp = vector >> 10;
    readslicef_11_1_10 = tmp[0:0];
  end
  endfunction


  function [10:0] readslicef_12_11_1;
    input [11:0] vector;
    reg [11:0] tmp;
  begin
    tmp = vector >> 1;
    readslicef_12_11_1 = tmp[10:0];
  end
  endfunction


  function [9:0] signext_10_1;
    input [0:0] vector;
  begin
    signext_10_1= {{9{vector[0]}}, vector};
  end
  endfunction


  function [0:0] readslicef_12_1_11;
    input [11:0] vector;
    reg [11:0] tmp;
  begin
    tmp = vector >> 11;
    readslicef_12_1_11 = tmp[0:0];
  end
  endfunction


  function [9:0] MUX_v_10_2_2;
    input [19:0] inputs;
    input [0:0] sel;
    reg [9:0] result;
  begin
    case (sel)
      1'b0 : begin
        result = inputs[19:10];
      end
      1'b1 : begin
        result = inputs[9:0];
      end
      default : begin
        result = inputs[19:10];
      end
    endcase
    MUX_v_10_2_2 = result;
  end
  endfunction


  function [0:0] readslicef_10_1_9;
    input [9:0] vector;
    reg [9:0] tmp;
  begin
    tmp = vector >> 9;
    readslicef_10_1_9 = tmp[0:0];
  end
  endfunction


  function [0:0] readslicef_8_1_7;
    input [7:0] vector;
    reg [7:0] tmp;
  begin
    tmp = vector >> 7;
    readslicef_8_1_7 = tmp[0:0];
  end
  endfunction


  function [3:0] MUX_v_4_2_2;
    input [7:0] inputs;
    input [0:0] sel;
    reg [3:0] result;
  begin
    case (sel)
      1'b0 : begin
        result = inputs[7:4];
      end
      1'b1 : begin
        result = inputs[3:0];
      end
      default : begin
        result = inputs[7:4];
      end
    endcase
    MUX_v_4_2_2 = result;
  end
  endfunction


  function [3:0] readslicef_5_4_1;
    input [4:0] vector;
    reg [4:0] tmp;
  begin
    tmp = vector >> 1;
    readslicef_5_4_1 = tmp[3:0];
  end
  endfunction


  function [0:0] readslicef_2_1_1;
    input [1:0] vector;
    reg [1:0] tmp;
  begin
    tmp = vector >> 1;
    readslicef_2_1_1 = tmp[0:0];
  end
  endfunction


  function [0:0] readslicef_9_1_8;
    input [8:0] vector;
    reg [8:0] tmp;
  begin
    tmp = vector >> 8;
    readslicef_9_1_8 = tmp[0:0];
  end
  endfunction


  function signed [11:0] conv_u2s_11_12 ;
    input [10:0]  vector ;
  begin
    conv_u2s_11_12 = {1'b0, vector};
  end
  endfunction


  function signed [11:0] conv_s2s_11_12 ;
    input signed [10:0]  vector ;
  begin
    conv_s2s_11_12 = {vector[10], vector};
  end
  endfunction


  function  [9:0] conv_s2u_9_10 ;
    input signed [8:0]  vector ;
  begin
    conv_s2u_9_10 = {vector[8], vector};
  end
  endfunction


  function  [7:0] conv_u2u_7_8 ;
    input [6:0]  vector ;
  begin
    conv_u2u_7_8 = {1'b0, vector};
  end
  endfunction


  function signed [5:0] conv_s2s_5_6 ;
    input signed [4:0]  vector ;
  begin
    conv_s2s_5_6 = {vector[4], vector};
  end
  endfunction


  function signed [4:0] conv_u2s_4_5 ;
    input [3:0]  vector ;
  begin
    conv_u2s_4_5 = {1'b0, vector};
  end
  endfunction


  function  [4:0] conv_u2u_4_5 ;
    input [3:0]  vector ;
  begin
    conv_u2u_4_5 = {1'b0, vector};
  end
  endfunction


  function signed [5:0] conv_u2s_5_6 ;
    input [4:0]  vector ;
  begin
    conv_u2s_5_6 = {1'b0, vector};
  end
  endfunction


  function  [1:0] conv_u2u_1_2 ;
    input [0:0]  vector ;
  begin
    conv_u2u_1_2 = {1'b0, vector};
  end
  endfunction


  function  [4:0] conv_s2u_3_5 ;
    input signed [2:0]  vector ;
  begin
    conv_s2u_3_5 = {{2{vector[2]}}, vector};
  end
  endfunction


  function  [3:0] conv_s2u_1_4 ;
    input signed [0:0]  vector ;
  begin
    conv_s2u_1_4 = {{3{vector[0]}}, vector};
  end
  endfunction

endmodule

// ------------------------------------------------------------------
//  Design Unit:    markers
//  Generated from file(s):
//    2) $PROJECT_HOME/../markers_source/blur.c
// ------------------------------------------------------------------


module markers (
  vin, vout_rsc_z, vga_xy, volume_rsc_z, clk, en, arst_n
);
  input [89:0] vin;
  output [29:0] vout_rsc_z;
  input [19:0] vga_xy;
  output [7:0] volume_rsc_z;
  input clk;
  input en;
  input arst_n;


  // Interconnect Declarations
  wire [29:0] vout_rsc_mgc_out_stdreg_d;


  // Interconnect Declarations for Component Instantiations 
  mgc_out_stdreg #(.rscid(2),
  .width(30)) vout_rsc_mgc_out_stdreg (
      .d(vout_rsc_mgc_out_stdreg_d),
      .z(vout_rsc_z)
    );
  mgc_out_stdreg #(.rscid(4),
  .width(8)) volume_rsc_mgc_out_stdreg (
      .d(8'b0),
      .z(volume_rsc_z)
    );
  markers_core markers_core_inst (
      .vin(vin),
      .vga_xy(vga_xy),
      .clk(clk),
      .en(en),
      .arst_n(arst_n),
      .vout_rsc_mgc_out_stdreg_d(vout_rsc_mgc_out_stdreg_d)
    );
endmodule



